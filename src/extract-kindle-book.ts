...
async function main(asin: string) {
  ...
  // Bind them after the assertions: the narrowing above does not reach into the
  // nested function below.
  const email: string = amazonEmail
  const secret: string = amazonPassword
  ...
  if (previous?.pages?.length && !previous.complete) {
    // Move the old attempt aside instead of deleting it: if this run fails
    // before capturing a single page — a login hiccup, a captcha — those pages
    // would be gone with nothing to show for it.
    const stale = `${pageScreenshotsDir}.stale`
    await fs.rm(stale, { recursive: true, force: true })
    await fs.rename(pageScreenshotsDir, stale).catch(() => {})
    console.warn(
      `setting aside ${previous.pages.length} pages from an incomplete earlier ` +
        `run of ${asin} (kept in ${path.basename(stale)} until this run succeeds)`
    )
  }
  ...
  /**
   * Closing a persistent context occasionally hangs, so give it a deadline.
   * Called from a finally block: without that, any assertion failure between
   * here and the end would leave a Chrome process and its profile lock behind,
   * and the next attempt at this book could not open the profile.
   */
  const closeBrowser = () =>
    pRace<void>((signal) => [
      (async () => {
  ...
  // Everything from here on is wrapped so the browser is closed even when an
  // assertion fails in the middle — otherwise a failed book leaves a Chrome
  // process running and its profile locked against the next attempt.
  try {
  ...
  async function extractWithBrowser() {
    ...
    async function updateSettings() {
      ...
      await page
        ...
        .click({ timeout: 15_000 })
        .catch(() =>
          console.warn('  ! font switch unavailable; keeping default')
        )
      await delay(200)
    ...
    async function recoverMetadataFromDisk() {
      ...
      for (const dir of renderDirs) {
        ...
        if (!result.toc?.length) {
          ...
          if (rawToc?.length) {
            result.toc = rawToc.flatMap((item) =>
              getTocItems(item, { depth: 0 })
            )
          }
        }
      }
      ...
      if (hasBookMetadata()) {
        console.warn(
          'recovered book metadata from the extracted render packages'
        )
      }
    }
    ...
    const noMetadataHint =
      `Could not load the book data for ${asin}. Most likely causes:\n` +
      `  - the ASIN is wrong, or the book is not in this Kindle library\n` +
      `  - the book is a sample or was returned\n` +
      `  - the login or 2FA did not go through\n` +
      `Open https://read.amazon.com/?asin=${asin} in a browser to check.`

    assert(result.meta, `${noMetadataHint}\n(missing: book metadata)`)
    assert(
      result.toc?.length,
      `${noMetadataHint}\n(missing: table of contents)`
    )
    assert(result.locationMap, `${noMetadataHint}\n(missing: location map)`)
    ...
    do {
      ...
      // Reassigned after a recovery reload, see below.
      let src = (await page
      ...
      do {
        ...
        // Snapshot the URL this attempt starts from, so the check below cannot
        // accidentally read a value updated by the recovery reload.
        const srcBeforeClick = src
          ...
          (signal) => [
            (async () => {
              while (!signal.aborted) {
                ...
                if (newSrc && newSrc !== srcBeforeClick) {
        ...
        if (retries === 8) {
          ...
          // The reload re-renders the same page under a fresh blob URL. Without
          // re-reading it, the "did we move on?" check below compares against the
          // pre-reload URL, always differs, and the same page is captured twice.
          const reloadedSrc = await page
            .locator(krRendererMainImageSelector)
            .getAttribute('src')
            .catch(() => undefined)
          if (reloadedSrc) src = reloadedSrc
        }
    ...
    // The previous attempt is only expendable once this one worked.
    if (result.complete) {
      await fs
        .rm(`${pageScreenshotsDir}.stale`, { recursive: true, force: true })
        .catch(() => {})
    }
    if (!result.complete) {
      console.warn(
...
for (const [i, asin] of asins.entries()) {
  ...
  try {
    // Compare explicitly: FORCE_REEXTRACT=0 is how people turn a flag back off,
    // and a plain truthiness check would read that as "yes, re-extract".
    const forceReextract = isTruthy(getEnv('FORCE_REEXTRACT'))

    if (!forceReextract && (await isAlreadyExtracted(asin))) {
      console.log(
